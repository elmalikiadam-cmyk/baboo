    ...
    <div className="space-y-4">
      {units.length > 0 ? (
        <ul className="space-y-2">
          {units.map((u) => (
            <li key={u.id} className="flex items-center gap-3 rounded-md border border-midnight/10 bg-cream p-3">
      ...
      <form onSubmit={onSubmit} className="rounded-md border border-border bg-cream p-4">
        ...
        <button
          type="submit"
          disabled={isPending}
          className="mono mt-3 rounded-full bg-midnight px-4 py-1.5 text-[10px] uppercase tracking-[0.12em] text-cream disabled:opacity-50"
        >

      ...
      {!hydrated ? (
        <ul className="mt-10 space-y-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <li key={i} className="h-32 animate-pulse rounded-md bg-cream-2" />
          ))}
        </ul>
      ) : items.length === 0 ? (
        <div className="mt-16 rounded-md border border-dashed border-border bg-cream-2/40 p-10 text-center">
...
function SavedSearchRow({
  ...
  return (
    <li className="rounded-md border border-border bg-cream p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-4">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-midnight text-cream">
            <SearchIcon className="h-5 w-5" />
          </span>
          <div className="min-w-0">
            <h2 className="display-lg text-xl md:text-2xl">{search.name}</h2>
            <p className="mono mt-1 text-[11px] uppercase tracking-[0.1em] text-muted">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-midnight/10 pt-4">

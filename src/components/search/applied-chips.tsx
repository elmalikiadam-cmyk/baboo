...
export function AppliedChips({ filters }: { filters: SearchFilters }) {
  ...
  if (filters.citySlug) {
    ...
    if (city) chips.push({ label: city.name, href: make({ citySlug: undefined, neighborhoodSlug: undefined }) });
  }
  ...
  if (filters.priceMin) chips.push({ label: `≥ ${formatMAD(filters.priceMin)}`, href: make({ priceMin: undefined }) });
  if (filters.priceMax) chips.push({ label: `≤ ${formatMAD(filters.priceMax)}`, href: make({ priceMax: undefined }) });
  if (filters.surfaceMin) chips.push({ label: `≥ ${filters.surfaceMin} m²`, href: make({ surfaceMin: undefined }) });
  if (filters.surfaceMax) chips.push({ label: `≤ ${filters.surfaceMax} m²`, href: make({ surfaceMax: undefined }) });
  if (filters.bedroomsMin) chips.push({ label: `${filters.bedroomsMin}+ ch.`, href: make({ bedroomsMin: undefined }) });
    ...
    <ul className="flex flex-wrap gap-2">
      {chips.map((c) => (
        <li key={c.label}>
          <Link
            href={c.href}
            className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-3 py-1 text-xs text-foreground hover:bg-foreground/5"
          >
            {c.label}

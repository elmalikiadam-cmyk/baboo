...
export default async function DeveloperProjects() {
  ...
  return (
    <div className="container py-10 md:py-16">
      ...
      {projects.length === 0 ? (
        <div className="mt-12 rounded-md border border-dashed border-foreground/25 p-10 text-center text-sm text-muted-foreground">
          Aucun projet. <Link href="/developer/projets/nouveau" className="font-medium text-foreground underline-offset-4 hover:underline">Créer le premier</Link>.
        </div>
      ) : (
        <ul className="mt-8 space-y-3">
          {projects.map((p) => (
            <li key={p.id} className="flex items-center gap-4 rounded-md border border-foreground/15 bg-surface p-4">
              <div className="relative h-16 w-24 shrink-0 overflow-hidden rounded-md bg-foreground/5">
              ...
              <div className="flex shrink-0 gap-2">
                <Link
                  href={`/projets/${p.slug}`}
                  className="mono rounded-full border border-foreground/20 px-3 py-1.5 text-[10px] uppercase tracking-[0.12em] hover:border-foreground"
                >
                  Voir
                </Link>
                <Link
                  href={`/developer/projets/${p.id}`}
                  className="mono rounded-full bg-foreground px-3 py-1.5 text-[10px] uppercase tracking-[0.12em] text-background hover:bg-foreground/90"
                >
                  Gérer
                </Link>
              </div>
